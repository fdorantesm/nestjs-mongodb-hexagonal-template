...
import { MeUseCase } from 'src/modules/auth/application/use-cases/me/me-use-case';
...
export class AuthController {
  ...
  public async login(@Body() credentials: LoginRequestDto) {
    const token = await this.loginUseCase.execute(credentials.email, credentials.password);
  ...
  public async me(@Req() request: UserRequest) {
    return await this.meUseCase.execute(request.user.id);
  }
}
